  ...
  passport.use(
    ...
    new LocalStrategy(
      ...
      async (email, password, done) => {
        try {
          const user = await userService.get_userby_Email(email);
          if (user == null) {
...
export const createUserTokens = (user: Omit<IUser, "password">) => {
  ...
  const jwtRSecret = process.env.JWT_REFRESH_TOKEN_SECRET ?? "";
  const token = jwt.sign(user, jwtSecret, {expiresIn: '30min'});
  const refreshToken = jwt.sign(user,jwtRSecret)
